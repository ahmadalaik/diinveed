...
export function Device({ iframeRef, device, snapshot }: Props) {
  const isDekstop = device === "desktop";

  const width = isDekstop ? DESKTOP_WIDTH : MOBILE_WIDTH;
  const height = isDekstop ? DESKTOP_HEIGHT : MOBILE_HEIGHT;
  const framePadding = isDekstop ? 10 : 12;
  const maxScale = isDekstop ? undefined : MOBILE_MAX_SCALE;
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            "relative shadow-2xl bg-black transition-all duration-300 ease-out",
            isDekstop ? "rounded-[20px]" : "rounded-[45px]",
          )}
          ...
          <div
            ...
            style={{
              ...
              clipPath: isDekstop
                ? "inset(0 round 10px)"
                : "inset(0 round 33px)",
              contain: isDekstop ? undefined : "paint",
            }}
          >
            <PreviewIframe
