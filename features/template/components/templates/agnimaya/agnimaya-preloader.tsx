...
export function LoadingAgnimaya({ inv, onDone, preload }: Props) {
  ...
  return (
    <div
      ...
      aria-busy={!fading}
      className={cn(
        "fixed inset-0 z-60 flex flex-col items-center justify-center gap-6 bg-stone-900 transition-opacity duration-600 ease-in-out",
    ...
    >
      ...
      <h1 className="text-4xl text-stone-50 font-(family-name:--tpl-font-heading) drop-shadow-lg">
        {name}
      </h1>
      <span
        className="size-2 rounded-full bg-(--tpl-bg-secondary) animate-pulse"
        aria-hidden
      />
