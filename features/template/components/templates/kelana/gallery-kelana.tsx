...
export function GalleryKelana({ inv, openLightbox }: Props) {
  ...
  return (
    <section ref={ref} className="px-2 py-16 bg-stone-100">
      ...
      >
        <h2 className="font-serif font-medium text-3xl text-[#2c2c2c] tracking-tight mb-1">
          Gallery
        </h2>
        <p className="text-base text-stone-500 font-serif font-light italic leading-relaxed mb-6">
          Our Moments
        </p>
      </motion.div>
      ...
      >
        {inv.gallery.filter((g) => g.url).map((gallery, index) => (
          <motion.div
            ...
            onMouseEnter={() =>
              preloadImage(cldUrl(gallery.url, "f_auto,q_auto,w_1080"))
            }
          >
            <Image
              fill
