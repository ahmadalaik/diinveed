...
function StoryCard({
  ...
  return (
    <motion.div
      ref={ref}
      className="flex flex-col justify-center text-center items-center gap-2 hover:border-[#6b7c62]/30 group"
      initial="hidden"
        ...
        <div className="space-y-6">
          {inv.stories.map((story, i) => (
            <StoryCard key={story.id} delay={i * 0.15}>
              <div className="text-[#6b7c62] mb-3 flex justify-center items-center p-2 rounded-full border border-stone-300 bg-[#fcfbf9]/35">
                <Milestone strokeWidth={1.5} />
              </div>
              <div className="pb-8">
                <h3 className="font-script font-medium tracking-wider text-2xl text-[#2c2c2c]">
                  {story.title}
                </h3>
                <p className="text-xs font-semibold text-[#6b7c62] uppercase tracking-widest mt-2 mb-4">
