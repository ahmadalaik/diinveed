...
export function RsvpAttendanceInput({ control, themeStyle }: Props) {
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel
              htmlFor="attendance"
              className="text-[10px] uppercase tracking-widest ml-1 transition-colors text-(--tpl-text-secondary)/80 group-focus-within:text-(--tpl-btn-bg-secondary)"
            >
              Kehadiran
            </FieldLabel>
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                <SelectTrigger
                  ...
                  className={cn(
                    ...
                    "bg-(--tpl-bg-tertiary)/20 text-(--tpl-text-secondary) data-placeholder:text-(--tpl-text-secondary)/70 focus:ring-2 focus:ring-(--tpl-btn-bg-secondary)/20",
                ...
                >
                  <div className="flex items-center gap-3">
                    <CalendarCheck2
                      strokeWidth={1.5}
                      className={cn(
                        "size-4 text-(--tpl-text-secondary)/75",
                        "group-focus-within:text-(--tpl-btn-bg-secondary)",
                      )}
                    />
                ...
                >
                  ...
                  >
                    <span className="font-(family-name:--tpl-font-body) text-(--tpl-text-tertiary)">
                      Hadir
                    </span>
                  </SelectItem>
                  <SelectItem
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            ...
            >
              <FieldLabel
                className={cn(
                  "text-[10px] uppercase tracking-widest ml-1 transition-colors text-(--tpl-text-secondary)/80 group-focus-within:text-(--tpl-btn-bg-secondary)",
                )}
              >
                Jumlah Tamu
              </FieldLabel>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Users
                        strokeWidth={1.5}
                        className={cn(
                          "size-4 transition-colors text-(--tpl-text-secondary)/80",
                          "group-focus-within:text-(--tpl-btn-bg-secondary)",
                        )}
                      />
