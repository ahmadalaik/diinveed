...
export function DikaraPreloader({ inv, onDone, preload }: Props) {
  ...
  return (
    <div
      ...
      aria-busy={!fading}
      className={cn(
        "fixed inset-0 z-60 flex flex-col items-center justify-center gap-6 bg-(--tpl-bg-secondary) transition-opacity duration-600 ease-in-out",
        fading ? "opacity-0" : "opacity-100",
      )}
    >
      <p className="text-xs tracking-[0.2em] uppercase text-(--tpl-text-secondary)/60">
        The Wedding of
      </p>
      <h1 className="text-4xl text-(--tpl-text-secondary) font-(family-name:--tpl-font-heading) drop-shadow-sm text-center px-4">
        {name}
      </h1>

      <div className="flex flex-col items-center gap-3 mt-4 w-full max-w-xs px-8">
        <p className="text-[10px] tracking-widest text-(--tpl-text-secondary)/50 font-(family-name:--tpl-font-body)">
          {progress}%
        </p>
        <div className="w-32 h-px bg-(--tpl-text-secondary)/10 overflow-hidden rounded-full">
          <div
