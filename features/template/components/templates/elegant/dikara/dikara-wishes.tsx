...
const DUMMY_WISHES: PublicWish[] = [
  ...
  },
  {
    id: "dummy-3",
    name: "Citra",
    wish: "Selamat yaa semoga bahagia selalu bersama pasangan.",
...
export function DikaraWishes({ publicToken, showCategory, mode }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <p className="text-[10px] text-(--tpl-text-tertiary)/90 text-balance uppercase tracking-[0.2em]">
      ...
      <div className="max-w-md mx-auto space-y-4">
        {wishes.length === 0 && !loading ? (
          <p className="text-center text-sm text-(--tpl-text-primary)/60 py-8">
          ...
          wishes.map((w) => (
            <div
              key={w.id}
              className="rounded-md bg-white/5 border border-white/10 shadow-sm p-4 backdrop-blur-sm"
            >
              <div className="flex items-center gap-2 mb-1">
                <MessageCircleHeart
                  strokeWidth={1.5}
                  className={cn("size-4", "text-(--tpl-text-primary)/60")}
                />
        ...
        {!isPreview && page < totalPages && (
          <div className="text-center pt-4">
            <Button
              ...
              onClick={loadMore}
              disabled={loading}
              className="text-xs uppercase tracking-widest border-(--tpl-text-primary)/40 text-(--tpl-text-primary) hover:bg-white/10"
            >
