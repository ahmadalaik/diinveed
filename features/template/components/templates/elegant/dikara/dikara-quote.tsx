...
export function DikaraQuote({ inv }: Props) {
  return (
    <section className="relative min-h-svh w-full overflow-hidden flex flex-col items-center px-8 pt-22 pb-10 justify-between snap-start snap-always">
      {inv.coverMobileImage && (
        <>
          <Image
            src={inv.coverMobileImage}
            alt="Prolog Background"
            fill
            sizes="100vw"
            quality={100}
      ...
      <motion.div
        ...
        custom={0.1}
      >
        <p className="text-4xl tracking-[0.2em] mb-4">
          {inv.isBrideFirst
            ? inv.brideNickname?.charAt(0)
            : inv.groomNickname?.charAt(0)}
        </p>
        <div
          className={cn("w-px h-14 rotate-25", "bg-(--tpl-text-primary)/60")}
        />
        <p className="text-4xl tracking-[0.2em] mt-4">
          {inv.isBrideFirst
            ? inv.groomNickname?.charAt(0)
            : inv.brideNickname?.charAt(0)}
        </p>
