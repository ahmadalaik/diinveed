...
export function PradiptaPreloader({ inv, onDone, preload }: Props) {
  ...
  return (
    ...
    >
      <p className="text-xs tracking-[0.2em] uppercase text-(--tpl-text-primary)/60">
        The Wedding of
      </p>
      <h1 className="text-4xl text-(--tpl-text-primary) font-(family-name:--tpl-font-heading) drop-shadow-sm text-center px-4">
        {name}
      </h1>

      <div className="flex flex-col items-center gap-3 mt-4 w-full max-w-xs px-8">
        <p className="text-[10px] tracking-widest text-(--tpl-text-primary)/50 font-(family-name:--tpl-font-body)">
          {progress}%
        </p>
        <div className="w-32 h-px bg-(--tpl-text-primary)/10 overflow-hidden rounded-full">
          <div
            className="h-full bg-(--tpl-text-primary)/80 transition-all duration-75 ease-out"
      ...
      <div className="absolute bottom-10 left-0 w-full flex flex-col items-center gap-1.5 opacity-90">
        <span className="text-[8px] uppercase tracking-[0.3em] text-(--tpl-text-primary)/50 font-(family-name:--tpl-font-body)">
          Powered by
        </span>
        <span className="text-xs tracking-widest text-(--tpl-text-primary)/80 font-(family-name:--tpl-font-heading)">
          Diinveed
        </span>
      </div>
