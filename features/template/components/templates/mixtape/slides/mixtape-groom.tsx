...
type MixtapeGroomProps = { inv: InvitationState };

export function MixtapeGroom({ inv }: MixtapeGroomProps) {
  const name = inv.groomName;
  const description = inv.groomDescription;
  const image = inv.groomImage;

  return (
    <section className="relative flex h-full flex-col justify-center px-6">
      <Doodle variant="burst" className="right-[2%] top-[8%] w-[36%]" />
      <MixtapeHeading thin="Mempelai" bold="Pria" className="text-3xl" />
