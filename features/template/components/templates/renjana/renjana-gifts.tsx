  ...
  return (
    <section className="px-8 py-16 border-b border-[#f0d6d6]">
      <motion.div
        ...
        variants={fadeUp(0)}
      >
        <h2 className="font-serif font-medium text-3xl text-[#a85d6b] tracking-tight">
          Gifts
        </h2>
        <p className="mt-3 px-6 text-base text-[#9a7e7e] font-serif font-light leading-relaxed">
      ...
      <div className="space-y-6">
        {(inv.gifts.transfers || []).map((gift, index) => (
          <GiftCard key={gift.id} delay={index * 0.15}>
            <h4 className="text-xl text-[#a85d6b] mb-6 flex items-center gap-3 font-normal font-serif">
        ...
        {(inv.gifts.packages || []).map((pkg, index) => (
          <GiftCard key={pkg.id} delay={((inv.gifts.transfers || []).length + index) * 0.15}>
            <h4 className="text-xl text-[#a85d6b] mb-6 flex items-center gap-3 font-normal font-serif">
              <Gift strokeWidth={1.5} className="text-[#c98a96]" />
              Kirim Kado
            </h4>
            <div className="space-y-3 text-sm text-[#6b4a4a] tracking-wide">
              <div className="flex flex-col items-start">
                <span className="font-light">Penerima</span>
                <span className="font-medium">{pkg.recipientName} ({pkg.recipientPhoneNumber})</span>
              </div>
              <div className="h-px bg-[#e8c9c9]" />
              <div className="flex flex-col items-start mb-2">
                ...
                <div className="flex items-start justify-between w-full gap-4">
                  ...
                  <CopyButton
                    className="bg-stone-50 hover:bg-[#c98a96] text-[#a85d6b] hover:text-stone-50 transition-all duration-500 tracking-wider text-xs uppercase rounded-sm shadow-sm font-medium shrink-0"
                    value={pkg.address}
                  />
                </div>
              </div>
